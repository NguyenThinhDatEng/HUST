...
priority_queue<myData, vector<myData>, compare> p_q;
...
void input() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        cin >> tmp1 >> tmp2;
        total_enemy += tmp1;
        p_q.push(myData(tmp1, tmp2));
    }
}

void solve() {
    while (!p_q.empty() && s > 0) {
        myData inf = p_q.top(); p_q.pop();
